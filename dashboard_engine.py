...
Toutes les fonctions utilitaires sont au niveau module.
run_analysis(ticker) est le point d'entrée Streamlit.
"""
...
# ── Palette + rcParams + toutes les fonctions utilitaires ─────
...
def run_analysis(TICKER, progress_cb=None):
    """Lance l'analyse complète. Retourne dict(html=..., ...)."""
    def _prog(p, msg=""):
    ...
    s_growth,  d_growth  = score_growth_v4(sp,rev_cagr3,eps_cagr3,fcf_cagr3,rev_growth_yoy)
    s_profit,  d_profit  = score_profitability_v4(sp,roe,roic,roa,roce)
    ...
    b64_mc=fig_to_b64(fig2); print("  ✓ Monte Carlo")
    ...
    *,*::before,*::after{{box-sizing:border-box;margin:0;padding:0}}
    ...
    return dict(
        ticker=TICKER, html=html,
        company=company, sector=sector,
        global_score=global_score, fund_score=fund_score,
        quant_score_100=quant_score_100,
        grade_g=grade_g, grade_f=grade_f, grade_q=grade_q,
        pos_sigma=pos_sigma, pos_zone=pos_zone, sig_txt=sig_txt,
        price=price, last_date=last_date,
    )
