...
def generate_dashboard(ticker, dcf_tv_growth=0.025, dcf_bear_gr=0.03, dcf_base_gr=0.08,
    ...
    div_yield    = div_yield_raw/100.0 if not np.isnan(div_yield_raw) and div_yield_raw > 1 else div_yield_raw
            ...
            try:
                ...
                if _is_rate_limit_error(e):
                    st.error(
                        ...
                        f"**{ticker_input}**. Réessaie dans 1 à 2 minutes — l'app retente déjà "
                        f"automatiquement plusieurs fois avant d'afficher cette erreur."
                    )
                else:
