...
# ═══════════════════════════════════════════════════════════════
#  SESSION YAHOO FINANCE "ANTI RATE-LIMIT"
#  Yahoo bloque agressivement les requêtes qui ne ressemblent pas à
#  un vrai navigateur (et les IP partagées type Streamlit Cloud sont
#  plus souvent limitées). curl_cffi imite l'empreinte TLS de Chrome,
#  ce qui réduit fortement les erreurs "Too Many Requests".
# ═══════════════════════════════════════════════════════════════
...
YF_SESSION = _build_yf_session()

def _is_rate_limit_error(exc) -> bool:
    msg = str(exc).lower()
    return any(k in msg for k in ["rate limit", "too many requests", "429"])

def _yf_call(fn, *args, max_retries=4, base_delay=6, **kwargs):
    """Exécute un appel réseau yfinance/yahooquery avec retry + backoff
    en cas de rate-limit Yahoo. Relance immédiatement les autres erreurs."""
    last_err = None
...
def build_tech_chart(ticker, last_date, last_price, years=5):
    ...
    raw_w = _yf_call(yf.download, ticker, start=start.strftime("%Y-%m-%d"),
                     interval="1wk", progress=False, session=YF_SESSION)
    ...
    raw_m = _yf_call(yf.download, ticker, period="max", interval="1mo",
...
def compute_wacc_dynamic(info, inc_df, total_debt, total_equity, market_cap):
    try:
        tnx = _yf_call(yf.download, "^TNX", period="5d", progress=False, session=YF_SESSION)
...
def generate_dashboard(ticker, dcf_tv_growth=0.025, dcf_bear_gr=0.03, dcf_base_gr=0.08,
    ...
    info = _yf_call(lambda: tk.info) or {}

    # tk.info (endpoint Yahoo "quoteSummary") est historiquement le point le plus
    # fragile de yfinance : il arrive qu'il revienne vide ou partiel alors que les
    # autres endpoints (historique de prix, états financiers) fonctionnent très bien.
    # On tente un filet de sécurité via fast_info (endpoint différent, plus robuste)
    # avant de se rabattre plus loin sur les données déjà téléchargées.
    fast_info = {}
    try:
        fi = _yf_call(lambda: tk.fast_info, max_retries=2, base_delay=4)
        for k in ["lastPrice", "marketCap", "shares", "currency"]:
            try:
                v = fi[k]
                if v is not None:
                    fast_info[k] = v
    ...
    info_was_degraded = len(info) < 5  # tk.info quasi vide -> Yahoo a bloqué/dégradé cet endpoint

    price      = safe(info.get("currentPrice") or info.get("regularMarketPrice"))
    if np.isnan(price):
        price = safe(fast_info.get("lastPrice"))
    market_cap = safe(info.get("marketCap"))
    if np.isnan(market_cap):
        market_cap = safe(fast_info.get("marketCap"))
    shares_out = safe(info.get("sharesOutstanding"))
    if np.isnan(shares_out):
        shares_out = safe(fast_info.get("shares"))
    sector     = info.get("sector","N/A")
    industry   = info.get("industry","N/A")
    company    = info.get("longName", TICKER)
    currency   = info.get("financialCurrency") or fast_info.get("currency") or "USD"

    print(f"  {company} ({TICKER}) | {sector} → multiplicateurs sectoriels chargés")
    print(f"  Prix : ${price:.2f} | Market Cap : {fmt_big(market_cap)}")

    raw = _yf_call(yf.download, TICKER, period="max", interval="1d",
                   progress=False, session=YF_SESSION)
    ...
    # Dernier filet de sécurité : le prix issu de l'historique (raw) est quasiment
    # toujours disponible même quand tk.info est dégradé. On l'utilise pour le prix
    # "officiel" affiché/utilisé dans le DCF si tk.info n'a rien donné, et on peut
    # recalculer une capitalisation approximative si on a au moins le nombre d'actions.
    if np.isnan(price):
        price = last_price
    ...
    def get_df(attr):
        try:
            df = _yf_call(lambda: getattr(tk, attr), max_retries=2, base_delay=4)
    ...
    total_debt   = safe(info.get("totalDebt"))     or row1(bal,"Total Debt","Long Term Debt","Long Term Debt And Capital Lease Obligation")
    total_cash   = safe(info.get("totalCash"))     or row1(bal,"Cash And Cash Equivalents","Cash Cash Equivalents And Short Term Investments")
    ...
    if np.isnan(roe) and not np.isnan(total_equity) and total_equity != 0 and not np.isnan(safe(ni_arr[0])):
        roe = safe(ni_arr[0]) / total_equity
    roa        = safe(info.get("returnOnAssets"))
    if np.isnan(roa) and not np.isnan(total_assets) and total_assets != 0 and not np.isnan(safe(ni_arr[0])):
        roa = safe(ni_arr[0]) / total_assets
    gross_margin= safe(info.get("grossMargins"))
    if np.isnan(gross_margin) and len(gm_hist.dropna()):
        gross_margin = float(gm_hist.dropna().iloc[-1])
    op_margin  = safe(info.get("operatingMargins"))
    if np.isnan(op_margin) and len(om_hist.dropna()):
        op_margin = float(om_hist.dropna().iloc[-1])
    net_margin = safe(info.get("profitMargins"))
    if np.isnan(net_margin) and len(nm_hist.dropna()):
        net_margin = float(nm_hist.dropna().iloc[-1])
    ...
    # Yahoo renvoie dividendYield tantôt en fraction (0.035 = 3.5%), tantôt déjà en
    # pourcentage (3.5 = 3.5%) selon le ticker/l'endpoint. Un vrai rendement dépasse
    # très rarement 100% (fraction > 1) : si c'est le cas, on considère que la valeur
    # est déjà en points de pourcentage et on la ramène en fraction.
    div_yield_raw = safe(info.get("dividendYield"))
    if not np.isnan(div_yield_raw) and div_yield_raw > 1:
        div_yield = div_yield_raw / 100.0
    else:
...
<div class="wrap">
  ...
  {'<div style="background:rgba(251,191,36,.08);border:1px solid rgba(251,191,36,.30);border-left:3px solid var(--amber);border-radius:9px;padding:12px 16px;margin-bottom:18px;font-size:12.5px;color:#FDE68A">⚠ Yahoo Finance a renvoyé des données incomplètes pour ce ticker (endpoint de profil/ratios dégradé). Certains indicateurs (PE, PB, PEG, EV/EBITDA, ROE, ROA, marges, ratios de bilan, dividende) peuvent être en N/A ou estimés à partir des états financiers plutôt que du profil Yahoo. Réessaie plus tard ou recharge la page — c\'est un problème côté Yahoo, pas un bug de ce tableau de bord.</div>' if info_was_degraded else ''}
  <div class="kpis">
  ...
  <footer>
    ...
    summary = dict(
        ...
        info_was_degraded=info_was_degraded,
    )
    return html, summary
        ...
        with st.spinner(f"Analyse de {ticker_input} en cours… (30 s à 2 min selon le ticker)"):
            try:
                ...
                if _is_rate_limit_error(e):
                    st.error(
                        f"❌ Yahoo Finance limite temporairement les requêtes (rate limit) pour "
                        f"**{ticker_input}**. Ce n'est pas un bug de l'app : Yahoo bride parfois les IP "
                        f"partagées comme celles de Streamlit Cloud. Réessaie dans 1 à 2 minutes — "
                        f"l'app retente déjà automatiquement plusieurs fois avant d'afficher cette erreur."
                    )
                else:
...
if st.session_state.dashboard_html:
    ...
    if summary.get("info_was_degraded"):
        st.warning(
            "⚠️ Yahoo Finance a renvoyé des données incomplètes pour ce ticker (endpoint de profil "
            "dégradé, indépendant de l'app). Certains ratios (PE, PB, PEG, EV/EBITDA, ROE, ROA, marges, "
            "dividende...) peuvent être en N/A ou approximés à partir des états financiers. "
            "Réessaie dans quelques minutes si tu veux les données complètes."
        )
